...
enum class DemoMode { PLAINSWITCH, PLAINBLINK, PLAINFADE, SNAKE, SNAKEFADE };

static DemoMode demoModes[] = {
    DemoMode::PLAINSWITCH, DemoMode::PLAINFADE, DemoMode::PLAINBLINK, DemoMode::PLAINFADE,
    //    DemoMode::SNAKE,
    //    DemoMode::SNAKEFADE
};
...
void Fixture::refreshPixelsDemo() {
    ...
    demoPrevModeMillis += (modeAdvance / (DEMO_SWITCHTIME * ARRAYSIZE(demoColors))) *
                          (DEMO_SWITCHTIME * ARRAYSIZE(demoColors));
...
void Fixture::refreshPixelsDemoPlainFade(int colorAdvance) {
    ...
    int green = ((int)demoColors[demoCurrentColorIdx] >> 8) & 0xff;
    int blue = ((int)demoColors[demoCurrentColorIdx]) & 0xff;
    red = (red * ((DEMO_SWITCHTIME / 2) - abs((DEMO_SWITCHTIME / 2) - colorAdvance))) /
          (DEMO_SWITCHTIME / 2);
    green = (green * ((DEMO_SWITCHTIME / 2) - abs((DEMO_SWITCHTIME / 2) - colorAdvance))) /
            (DEMO_SWITCHTIME / 2);
    blue = (blue * ((DEMO_SWITCHTIME / 2) - abs((DEMO_SWITCHTIME / 2) - colorAdvance))) /
           (DEMO_SWITCHTIME / 2);
    for (int i = 0; i < _pixelCount; ++i) {
        _pixels[i] = CRGB(red, green, blue);
    }
...
void Fixture::refreshPixelsDemoSnake(int colorAdvance) {
    ...
    int blue = ((int)demoColors[demoCurrentColorIdx]) & 0xff;
    int position =
        (_pixelCount * ((DEMO_SWITCHTIME / 2) - abs((DEMO_SWITCHTIME / 2) - colorAdvance))) /
        (DEMO_SWITCHTIME / 2);
    for (int i = 0; i < _pixelCount; ++i) {
        if (abs(position - i) < 4) {
            _pixels[i] = CRGB(red, green, blue);
